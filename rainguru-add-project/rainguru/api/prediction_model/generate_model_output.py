...
def run_model(start_datetime, data_dir, model_name = "all_data_balanced_loss_iteration100", iteration_number = 68000, pixel_type='normalised pixel values'):
    ...
    # Hardcoded for now, could not find any other way of making this work, even with environment variables
    encoder_forecaster.load_state_dict(torch.load(f"api/prediction_model/{model_name}/models/encoder_forecaster_{iteration_number}.pth", map_location=torch.device(cfg.GLOBAL.DEVICE)))
    ...
    datetime_clip = pd.date_range(start=start_datetime, periods=IN_LEN, freq='5min')
...
def load_frames(datetime_clips, data_dir):
    ...
    """
    frame_dat = np.zeros((cfg.HKO.BENCHMARK.IN_LEN, 1, 1, 480, 480))
